...
const RecipeCard = ({ id, name, difficulty, tags, image }: SinglePreview) => {
  return (
    <div key={`card-${id}`} className="max-w-sm bg-white rounded-2xl">
      <Link to={`recipe/${id}`}>
        <img
          className="aspect-2/1 w-full rounded-t-2xl object-cover"
          src={image}
          alt=""
        />
      </Link>
